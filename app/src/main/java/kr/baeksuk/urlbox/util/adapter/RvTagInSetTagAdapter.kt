...
class RvTagInSetTagAdapter(
    ctx: Context,
    act: Activity,
    private val tagClickedListener: OnTagSelectedListener
) : RecyclerView.Adapter<RvTagInSetTagAdapter.MyViewHolder>() {
    ...
    fun setTagData(tagDataList: List<Tag>) {
        ...
        tagList = tagDataList.distinct()

        notifyDataSetChanged()
    }


    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RvTagInSetTagAdapter.MyViewHolder {
        val binding = ItemTagInCaptureListBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return MyViewHolder(binding)
    }

    override fun onBindViewHolder(holder: RvTagInSetTagAdapter.MyViewHolder, position: Int) {
