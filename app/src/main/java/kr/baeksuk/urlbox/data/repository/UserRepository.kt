...
class UserRepository(application: Application) {
    ...
    **/
    ...
    fun insertUrlData(url: List<Url>, userId: String) {

        val urlInUser: DatabaseReference = database.child("User").child(userId).child("url")

        urlInUser.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {

                urlInUser.setValue(url)

            }

            override fun onCancelled(error: DatabaseError) {

                Log.e("url 동기화", error.toString())
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    ...
                    if (newUrls.isNotEmpty()) {
                        // 중복되지 않은 데이터만 추가
                        val combinedUrls = existingUrls + newUrls
                        urlInUser.setValue(combinedUrls).addOnSuccessListener {
                            Log.e("URL 데이터 저장", "새로운 URL 저장 성공")
                        }.addOnFailureListener {
                            Log.e("URL 데이터 저장", "URL 데이터 저장 실패")
                        }
                    } else {
                        Log.e("URL 데이터 저장", "모든 URL이 중복되어 저장하지 않음")
                    }
                } else {
                    // 처음 로그인한 경우, 유저 정보 저장 후 URL 추가
                    userRef.setValue(user).addOnSuccessListener {
                        Log.e("유저 아이디 저장", "유저 아이디 저장 성공")
                        urlInUser.setValue(urlList) // 새 URL 데이터 저장
                    }.addOnFailureListener {
                        Log.e("유저 아이디 저장", "유저 아이디 저장 실패")
                    }
                }
            }
